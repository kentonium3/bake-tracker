...
## Work Package WP01: Service Primitives (Priority: P1) 🎯 Foundation
...
### Risks & Mitigations
- **Risk**: Session detachment when accessing relationships → **Mitigation**: Eager load FinishedUnits in query
...
### Risks & Mitigations
- **Risk**: Breaking existing callers → **Mitigation**: New param has default value `None`
- **Risk**: Slug collision → **Mitigation**: Include variant recipe ID in slug generation
...
### Risks & Mitigations
- **Risk**: No existing variant UI → **Mitigation**: Research existing recipe form patterns first
- **Risk**: Complex dialog state → **Mitigation**: Keep state minimal, validate on save
...
### Risks & Mitigations
- **Risk**: Many display locations → **Mitigation**: Focus on production dashboard first, expand if time permits
