...
### Risks & Mitigations
- Existing tests may fail with nullable changes → Update test fixtures
- Breaking existing recipe creation → Validation only warns initially
...
### Risks & Mitigations
- Malformed JSON in test data → Add validation and error messages
...
### Risks & Mitigations
- Missing recipe reference → Use recipe.name as FK lookup key
...
### Risks & Mitigations
- Duplicate FinishedUnit creation → Check for existing before creating
- Recipe not found → Skip with warning, continue import

---

## Work Package WP05: UI Updates (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Data loss → Always backup before transformation
...
```
WP01 (Foundation) ──────┬───► WP03 (Export) ──────┐
                        │                          │
WP02 (Transform) ───────┼───► WP04 (Import) ──────┼───► WP06 (Integration)
                        │                          │
                        └───► WP05 (UI) ───────────┘
```

- **Sequence**: WP01 → WP03/WP04 → WP05 → WP06
