...
2. Update relationship: `material` → `material_product` with `lazy="joined"`
...
### Risks & Mitigations
- Schema change breaks existing tests → Run tests after each model change to catch issues early
...
### Risks & Mitigations
- Breaking existing Compositions with material_id → Handled by migration script (WP09)
...
- [x] T017 Update MaterialUnit service tests in `src/tests/test_material_unit_service.py`
...
### Risks & Mitigations
- Service calls may fail if models not updated → Verify model changes before starting

---

## Work Package WP04: Auto-Generation in MaterialProduct Service (Priority: P1) 🎯 MVP
...
- [x] T022 Add MaterialProduct service tests for auto-generation in `src/tests/test_material_product_service.py`
...
### Risks & Mitigations
- Auto-generation creates unwanted units → Allow deletion if not referenced
...
### Risks & Mitigations
- Existing code may call removed methods → Search for usages before removing
...
**Independent Test**: Export→Import round-trip preserves MaterialUnit relationships via material_product_slug.
...
- [x] T031 Add export/import tests for MaterialUnit with new FK in `src/tests/test_import_export_service.py`
...
### Risks & Mitigations
- Existing exports may have material_slug → Migration script handles transformation
...
### Risks & Mitigations
- UI pattern mismatch → Study Recipe→FinishedUnits pattern before implementing
...
### Risks & Mitigations
- Existing "Add Unit" functionality may be relied upon → Document in release notes
...
**Independent Test**: Script transforms MaterialUnits from material_slug to material_product_slug with N×M duplication.
...
- [x] T044 Implement Composition material_id skip logging in `scripts/migrate_material_units.py`
- [x] T045 Add migration script tests in `src/tests/test_migrate_material_units.py`
...
### Risks & Mitigations
- Migration creates too many duplicates → Log count for user awareness
