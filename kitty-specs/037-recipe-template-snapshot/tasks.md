...
WP01 (Models) ─────────────────────────────────────────────────────────────
   │
   ├──→ WP02 (Snapshot Service) ──→ WP03 (Production Integration) ──→ WP04 (Migration)
   │         │                              │
   │         └──→ WP08 (History View)       └──→ WP05 (Scale Factor UI)
   │
   ├──→ WP06 (Variant Service & UI) [PARALLEL with WP02]
   │
   └──→ WP07 (Production Readiness) [PARALLEL with WP02, WP06]
```
...
## Work Package WP01: Models Layer (Priority: P1) 🎯 MVP Foundation
...
### Risks & Mitigations
- Self-referential FK complexity → Use explicit CHECK constraint
- JSON column in SQLite → Use Text type with json.dumps/loads

---

## Work Package WP02: Snapshot Service (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Session detachment → Follow CLAUDE.md patterns exactly
- JSON serialization errors → Add validation and error handling

---

## Work Package WP03: Production Integration (Priority: P1) 🎯 MVP
...
- Scale factor affects ingredient quantities: base_quantity × scale_factor × num_batches
...
- FIFO consumption errors → Preserve existing consume_fifo() logic
...
- Recipe deleted → Handle gracefully with warning
...
- Display: "Expected: 72 cookies (36 × 2.0 × 1 batch)"
...
### Risks & Mitigations
- UI confusion → Clear labeling and calculated preview
...
### Risks & Mitigations
- UI complexity → Follow existing ingredient hierarchy patterns
...
### Risks & Mitigations
- User confusion → Clear labeling, tooltip explaining purpose
...
### Risks & Mitigations
- Performance with many snapshots → Paginate if needed (defer)

---

## Dependency & Execution Summary

- **Sequence**: WP01 → (WP02 || WP06 || WP07) → (WP03 || WP08) → (WP04 || WP05)
- **Parallelization**:
