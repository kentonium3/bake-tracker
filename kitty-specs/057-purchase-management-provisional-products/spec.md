...
2. **Given** a user chooses to create a provisional product, **When** they provide brand, description, and select an ingredient, **Then** a provisional product is created with `needs_review=true` and the purchase is recorded against it.
...
2. **Given** a user is in the Products tab, **When** they select the "Needs Review" filter, **Then** only products with `needs_review=true` are displayed.
...
4. **Given** a user completes the missing fields for a provisional product, **When** they mark it as reviewed, **Then** the `needs_review` flag is cleared and the product no longer appears in the review queue.
...
- **FR-006**: System MUST create provisional products with `needs_review=true` flag.
- **FR-007**: System MUST delegate slug generation to product_catalog_service with uniqueness validation.
- **FR-008**: System MUST allow user to provide brand, description, and select ingredient for provisional products.
...
- **FR-014**: System MUST clear the `needs_review` flag when a user marks a product as reviewed.
...
- **Product**: An item in the product catalog. May have `needs_review=true` if created provisionally. Has a unique slug generated by product_catalog_service.

- **Provisional Product**: A Product created during purchase workflow with incomplete information. Flagged with `needs_review=true`. Fully usable for purchases while awaiting review.
...
- Product model supports a `needs_review` boolean field (or can be added).
...
- **Product model changes required**: Adding `needs_review` field may require migration.
