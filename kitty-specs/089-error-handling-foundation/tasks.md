...
## Work Package WP01: Exception Hierarchy Consolidation (Priority: P0) 🎯 Foundation
...
### Risks & Mitigations
- Breaking existing exception handling → maintain backward compatibility, don't change signatures
- Import errors → keep all exceptions in same location
...
### Risks & Mitigations
- Missing exception classes → grep for `class.*Error.*Exception` to find all
- Circular imports → import ServiceError at top, not locally
...
### Risks & Mitigations
- Missing exception mappings → add fallback for unmapped ServiceError subclasses
- Logging failures → wrap logging in try/except

---

## Work Package WP04: UI Migration - Core Tabs (Priority: P1) 🎯 MVP
...
- Apply three-tier pattern: specific exceptions → ServiceError → Exception
...
### Risks & Mitigations
- Different error display patterns → review current pattern before replacing
- Missing imports → add ServiceError import if catching specifically
...
### Risks & Mitigations
- Recipe validation has custom handling → preserve existing ValidationError handlers
...
### Risks & Mitigations
- Custom error display for dependencies → check for `.dependencies` or `.details` attribute display
...
### Risks & Mitigations
- Event detail is complex (19 occurrences) → may need careful review
...
### Risks & Mitigations
- Form validation feedback → preserve field-level error display if present
...
### Risks & Mitigations
- UPC dialog has special handling → preserve barcode-specific error messages
...
### Risks & Mitigations
- Dashboard errors should show partial data → consider try/except per component
...
### Risks & Mitigations
- Import/export is critical path → test thoroughly after changes
- CLI tools need text output not GUI → adapt handler for CLI context
...
### Risks & Mitigations
- Documentation drift → link to actual code, not copied snippets
...
### Risks & Mitigations
- Missing exception classes → dynamic discovery via imports
- Test failures → prioritize fixing over expanding tests

---

## Dependency & Execution Summary

- **Sequence**: WP01 → WP02 → WP03 → (WP04-WP12 in parallel) → WP13 → WP14
