...
### Risks & Mitigations
- F087 deviation → Copy directly from recipes_tab.py; verify trackpad scrolling works
...
### Risks & Mitigations
- Missed circular reference edge case → Test A→B→C→A cycle explicitly
...
### Risks & Mitigations
- Form layout complexity → Use grid layout with consistent padding

---

## Work Package WP05: Form Dialog - Foods Component Section (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Large FinishedUnit list → Category filter + search reduces options
...
- Circular reference missed in UI → Service layer validates again on save
...
WP05 (Form Foods) ──────────────┤                             │
    (depends on WP04)           │                             │
                                 ├───► WP07 (Tab Integration) ─┤
WP06 (Form Materials+Components)│         (depends on all)    │
