...
```
WP01 (Ingredient Display) ──┐
                            ├──► WP03 (Services) ──► WP04 (Admin UI) ──► WP05-WP07 (Operations) ──► WP08 (Polish)
WP02 (Material Display) ────┘
```
...
## Work Package WP01: Ingredient Display Changes (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Performance with many ingredients → eager-load parent relationships in single query

---

## Work Package WP02: Material Display Changes (Priority: P1) 🎯 MVP
...
### Implementation Notes
- Service joins Material → MaterialSubcategory → MaterialCategory
...
### Risks & Mitigations
- Similar to WP01 → use same pattern for consistency
...
### Risks & Mitigations
- Recursive tree queries → limit depth to 3 levels (known max)
...
### Risks & Mitigations
- ttk.Treeview styling → test on both Windows and macOS
...
### Risks & Mitigations
- Duplicate name error → show user-friendly validation message
...
### Risks & Mitigations
- Cycle creation → validate before save using `validate_no_cycle()`
...
### Implementation Notes
- Menu structure: Catalog → Hierarchy Admin → Ingredients / Materials
- Or: Catalog → Ingredients Admin / Materials Admin (simpler)
...
### Risks & Mitigations
- Menu location confusion → match existing Catalog mode patterns
...
├── WP01: Ingredient Display [P] ──┐
└── WP02: Material Display [P] ────┴──► Phase 2

Phase 2 (Services):
└── WP03: Hierarchy Admin Services ──► Phase 3

Phase 3 (UI Shell):
└── WP04: Hierarchy Admin UI Shell ──► Phase 4

Phase 4 (Operations - Can Parallel):
├── WP05: Add Operations [P] ──────┐
├── WP06: Rename Operations [P] ───┼──► Phase 5
└── WP07: Reparent Operations [P] ─┘
