...
- [x] T003 Implement `remove_event_fg()` in `src/services/event_service.py`
- [x] T004 Write unit tests in `src/tests/services/test_event_service_fg_quantities.py`
...
- Track quantities with `Dict[int, ctk.StringVar]` (fg_id → quantity string)
- Validation pattern: empty OK, positive integer OK, zero/negative/non-int → orange text
...
## Work Package WP03: Planning Tab Integration (Priority: P1) 🎯 MVP
...
- [x] T012 End-to-end validation: manual test of complete workflow
...
```
WP01 (Service) ──┬──► WP02 (UI) ──┬──► WP03 (Integration) 🎯 MVP
                 │                │
                 └────────────────┘
                    (both needed)
```
