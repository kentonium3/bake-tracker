...
## Work Package WP01: UI Session Infrastructure (Priority: P0) 🎯 Foundation
...
### Risks & Mitigations
- **Risk**: Import cycles between ui and services → **Mitigation**: Only import from `src.services.database`, not service modules
...
### Risks & Mitigations
- **Risk**: Many callers to update → **Mitigation**: Search for function names, update systematically
...
### Included Subtasks
- [ ] T011 Add required `session` param to: `assign_package_to_recipient`, `update_assignment`, `remove_assignment`
- [ ] T012 Add required `session` param to: `get_event_assignments`, `get_recipient_assignments_for_event`
- [ ] T013 Remove internal `session_scope()` from all assignment functions
...
### Risks & Mitigations
- **Risk**: Assignment dialog may have complex flows → **Mitigation**: Ensure single session wraps entire assignment operation
...
### Risks & Mitigations
- **Risk**: Complex aggregation queries → **Mitigation**: Verify all sub-queries use provided session
...
### Risks & Mitigations
- **Risk**: Complex session threading → **Mitigation**: Trace call chain carefully, test with uncommitted data
...
### Risks & Mitigations
- **Risk**: Status updates may trigger cascading changes → **Mitigation**: Ensure all cascades use same session
...
### Risks & Mitigations
- **Risk**: Clone operation complexity → **Mitigation**: Test clone with rollback to verify atomicity
...
### Risks & Mitigations
- **Risk**: Subtle bug may have other manifestations → **Mitigation**: Search for similar pattern across codebase
...
### Risks & Mitigations
- **Risk**: Confusion between production_service and batch_production_service → **Mitigation**: Clear file paths in prompts
...
### Risks & Mitigations
- **Risk**: UI code may expect Decimal → **Mitigation**: Search for `.quantize()` or Decimal operations on service returns
...
### Risks & Mitigations
- **Risk**: Logging overhead → **Mitigation**: Keep log statements minimal, use DEBUG for verbose info
...
WP01 (Foundation) ────────────────────────────────────────────────────►
         ...
         ├──► WP02 (Event CRUD) ─────────────────────────────────────►
         │
         ├──► WP03 (Event Assignment) ───────────────────────────────►
         │
         ├──► WP04 (Event Calculation) ──────────────────────────────►
         │
         ├──► WP05 (Event Progress) ─────────────────────────────────►
         │
         ├──► WP06 (Event Target/Status) ────────────────────────────►
         │
         ├──► WP07 (Event Remaining) ────────────────────────────────►
         │
         ├──► WP08 (Batch/Assembly Bugfix) ──────────────────────────►
         │
         └──► WP09 (Production Service) ─────────────────────────────►
                                                                      │
         After all services updated: ─────────────────────────────────┤
                                                                      │
                                         WP10 (DTO Standardization) ──►
                                                                      │
                                         WP11 (Structured Logging) ───►
```

### Parallelization Opportunities

After WP01 completes, these can run in parallel:
- **Stream A (Lead Agent)**: WP02 → WP03 → WP04 → WP05 → WP06 → WP07 (event_service chunks)
- **Stream B (Parallel Agent)**: WP08 → WP09 (other services)
