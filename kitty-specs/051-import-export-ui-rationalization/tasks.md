...
### Risks & Mitigations
- Risk: Inconsistent validation rules across entities → Review existing import logic for field requirements
...
### Risks & Mitigations
- Risk: Permission errors on directories → Validate write access; fall back gracefully
...
### Risks & Mitigations
- Risk: Main window file not obvious → Check `main_window.py` or menu setup location
...
### Risks & Mitigations
- Risk: Log file write failure → Handle permission errors; report to user
...
### Risks & Mitigations
- Risk: Slug field missing from model → Verify Supplier model has slug from F050
...
### Risks & Mitigations
- Risk: Duplicate slug conflicts → Validate and report clear error

---

## Work Package WP07: Unified Import Dialog with Context-Rich Purpose (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Risk: Regression in existing purposes → Test all 5 purposes after changes
...
### Risks & Mitigations
- Risk: Breaking change to menu structure → Test File menu after changes
...
Phase 0 (Foundational - Independent):
  WP01: Schema Validation Service ──┐
  WP02: Preferences Service ────────┼─→ Can start immediately in parallel
  WP05: Supplier Export ────────────┘

Phase 1 (Dependent Services):
  WP03: Preferences Dialog ─────────→ After WP02
  WP04: Enhanced Import Logging ────→ After WP02
  WP06: Supplier Import ────────────→ After WP05

Phase 2 (Integration):
  WP07: Unified Import Dialog ──────→ After WP01, WP04 (MVP)

Phase 3 (Cleanup):
  WP08: Menu Cleanup & Multi-Entity → After WP07
```
...
**MVP Scope**: WP01 → WP02 → WP04 → WP07 delivers unified import with validation. Other work packages enhance the experience but aren't blocking.
