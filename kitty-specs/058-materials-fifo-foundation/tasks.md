...
- base_unit_type migration: linear_inches→linear_cm, square_inches→square_cm
...
- Area conversions to square_cm: square_feet (×929.0304), square_inches (×6.4516), square_meters (×10000)
- Validation must reject cross-type conversions (e.g., feet → square_cm)
...
## Work Package WP05: MaterialInventoryService Core (Priority: P1) 🎯 MVP
...
- Second scenario: Lot A (30cm remaining), consume 50cm → 30cm from A + 20cm from B = $6.00
...
**Sequence**:
```
WP01 (Schema: Model) ──┬──► WP02 (Schema: Cleanup) ──► WP08 (Import/Export)
                       │                              └──► WP09 (UI Cleanup)
                       ├──► WP03 (Schema: Updates)
                       │
WP04 (Unit Converter) ─┴──► WP05 (Inventory Service) ──► WP06 (Service Tests)
                       │                              └──► WP07 (Purchase Integration)
                       │
...
| T040 | Test purchase→inventory flow | WP10 | P2 | Yes |
