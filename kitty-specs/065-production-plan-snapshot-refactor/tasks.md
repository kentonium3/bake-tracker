...
### Risks & Mitigations
- Risk: Other code references removed fields → Run grep for field names before removing
- Risk: Tests fail → Update/remove tests that reference removed fields
...
### Risks & Mitigations
- Risk: F064 didn't add planning_snapshot_id → Add it in this WP if missing
- Risk: Unique constraint issues → Ensure recipe_snapshot_id FK doesn't conflict with existing constraints
...
### Risks & Mitigations
- Risk: Session detachment → Follow CLAUDE.md session management patterns exactly
- Risk: Existing callers break → Ensure backward compatibility (production_run_id still works as before)

---

## Work Package WP04: Planning Service Snapshot Creation (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Risk: Partial snapshot creation on failure → Single transaction with rollback
- Risk: Session detachment → Pass session through all calls
- Risk: FinishedGoodSnapshot service not ready → Verify F064 completion first
...
### Risks & Mitigations
- Risk: Incorrect snapshot used → Match on (event_id, recipe_id) to find correct target
- Risk: Legacy production breaks → Maintain fallback to create-at-execution
...
### Risks & Mitigations
- Risk: F064 service not compatible → Verify create_finished_good_snapshot() returns reusable snapshot
- Risk: Recursive snapshot issue → FinishedGoodSnapshot already handles component snapshots
...
### Risks & Mitigations
- Risk: Performance regression → Profile typical event; optimize queries if needed
- Risk: Different results than cached → Verify calculation logic matches original
- Risk: Missing snapshot data → Handle gracefully (calculate from live definitions as fallback)
...
### Risks & Mitigations
- Risk: UI breaks on missing data → Handle empty/null gracefully
- Risk: Performance too slow → Add loading indicator while calculating
...
- Test full workflow: create event → create plan → record production → verify same snapshot
- Test legacy: record production for event without plan → new snapshot created
...
### Risks & Mitigations
- Risk: Integration test environment setup → Use existing test fixtures
- Risk: Coverage below 70% → Add additional unit tests for uncovered paths
...
Sequence:
  WP01, WP02 (parallel) → WP03, WP05, WP06 (parallel after WP02) → WP04 → WP07 → WP08 → WP09
```
...
| T040 | Integration test: plan→production | WP09 | P3 | Yes |
| T041 | Integration test: plan→assembly | WP09 | P3 | Yes |
