...
### Risks & Mitigations
- Session management bugs → Follow documented pattern exactly; test multi-step operations
- FIFO calculation errors → Unit test with consumed/unconsumed fixtures

---

## Work Package WP02: Purchases Tab - List & Filters (Priority: P1) 🎯 MVP

**Goal**: Create PurchasesTab with list view, filters, and dashboard integration (User Stories 1 & 2).
**Independent Test**: Navigate to PURCHASE mode → Purchases tab shows list with filters working.
...
### Risks & Mitigations
- Performance with 500+ rows → Use ttk.Treeview (proven pattern)
- Filter state sync → Apply all filters on any change

---

## Work Package WP03: Add Purchase Dialog (Priority: P1) 🎯 MVP

**Goal**: Implement Add Purchase dialog for primary data entry (User Story 3).
**Independent Test**: Click "Add Purchase" → fill form → save → purchase appears in list + inventory updated.
...
### Risks & Mitigations
- Product dropdown performance → Lazy load, limit to recent + filtered
- Validation UX → Show inline errors, don't close on failure
...
**Independent Test**: Select purchase → Edit → change price → save → list shows updated values.
...
### Risks & Mitigations
- FIFO recalculation confusion → Clear preview of impact before save
...
**Independent Test**: Try delete consumed purchase → blocked with details; delete unconsumed → succeeds.
...
**Independent Test**: View Details → shows purchase info + usage history + remaining inventory.
...
### Risks & Mitigations
- Performance with large usage history → Limit to 50 most recent
...
### Risks & Mitigations
- Last-minute bugs → Allow time for iteration
...
```
WP01 (Service Layer) ──┬──> WP02 (Tab UI) ───────> WP05 (Delete) ──┐
                       │                                            │
                       ├──> WP03 (Add Dialog) ─────────────────────┤
                       │                                            ├──> WP07 (Integration)
                       ├──> WP04 (Edit Dialog) [Gemini] ───────────┤
                       │                                            │
                       └──> WP06 (View Details) [Gemini] ──────────┘
```
