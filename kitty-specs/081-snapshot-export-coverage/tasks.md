...
### Included Subtasks
- [ ] T001 Add RecipeSnapshot model import and DEPENDENCY_ORDER entry
- [ ] T002 Implement `_export_recipe_snapshots()` function
- [ ] T003 Add FinishedGoodSnapshot model import and DEPENDENCY_ORDER entry
- [ ] T004 Implement `_export_finished_good_snapshots()` function
...
- MaterialUnit → material_unit_slug FK resolution
- FinishedUnit → finished_unit_slug FK resolution
...
- Resolve FK via slug lookup (recipe_slug → recipe_id)
...
## Work Package WP05: Comprehensive Test Suite (Priority: P1) 🎯 MVP Complete
...
```
WP01 (Recipe/FG Export) ──┐
                          ├──► WP03 (Recipe/FG Import) ──┐
WP02 (MU/FU Export) ──────┤                              ├──► WP05 (Tests)
                          └──► WP04 (MU/FU Import) ──────┘
```

- **Sequence**: WP01 → WP03 → WP05 (main path), WP02 can parallel with WP01
