  ...
  try:
      with session_scope() as session:
          ...
          if variant_data.get('preferred', False):
              session.query(Variant).filter_by(ingredient_id=ingredient.id).update({'preferred': False})
  ...
  try:
      with session_scope() as session:
          ...
          session.query(Variant).filter_by(
              ingredient_id=variant.ingredient_id
      ...
      raise DatabaseError(
          f"Failed to set preferred variant {variant_id}",
          original_error=e
      )
