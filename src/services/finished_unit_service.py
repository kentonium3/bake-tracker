...
class FinishedUnitService:
            ...
            try:
                ...
                if "uq_finished_unit_slug" in str(e) and attempt < max_retries - 1:
                    ...
                    if "uq_finished_unit_slug" in str(e):
                        raise DuplicateSlugError(f"Unable to generate unique slug after {max_retries} attempts")
                    else:
        ...
        try:
            with get_db_session() as session:
                ...
                if fifo_cost is None or fifo_cost == Decimal('0.0000'):
                    ...
                    logger.debug(f"Using recipe cost for '{unit.display_name}': {calculated_cost} (FIFO unavailable)")
                else:
        ...
        try:
            ...
            for recipe_ingredient in unit.recipe.ingredients:
                ...
                if ingredient_cost is not None:
                    ...
                    logger.debug(f"FIFO cost for {ingredient.name} ({quantity_needed} {ingredient.recipe_unit}): {ingredient_cost}")
                else:
    ...
    @staticmethod
    def _get_ingredient_fifo_cost(ingredient_slug: str, quantity_needed: Decimal) -> Optional[Decimal]:
        """
    ...
    @staticmethod
    def _get_pantry_item_unit_cost(pantry_item: PantryItem, quantity_consumed: Decimal,
                                  ingredient: 'Ingredient') -> Optional[Decimal]:
        """
