...
class UIServiceIntegrator:
    ...
    def execute_service_operation(
        ...
        try:
            ...
            self._log_with_level(
                log_level, f"Starting {operation_type.value} operation: {operation_name}"
            )
            ...
            self._log_with_level(
                log_level,
                f"Completed {operation_type.value} operation: {operation_name} "
            ...
            # Log the error
            logger.error(
                f"Failed {operation_type.value} operation: {operation_name} "
    ...
    def _get_user_friendly_error_message(
        ...
        if isinstance(exception, InvalidInventoryError):
            return (
                f"{ctx_prefix}Invalid inventory operation. Please check the quantity and try again."
            )
        ...
        if isinstance(exception, FinishedGoodNotFoundError):
            return (
                f"{ctx_prefix}The requested assembly could not be found. It may have been deleted."
            )
    ...
    def _record_failure(
        self, operation_name: str, execution_time: float, exception: Exception
    ) -> None:
    ...
    def get_operation_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            success_rate = (
                (self.operation_stats["successful_operations"] / total_ops * 100)
                if total_ops > 0
                else 0
            )
            failure_rate = (
                (self.operation_stats["failed_operations"] / total_ops * 100)
                if total_ops > 0
                else 0
            )
            ...
            if self.operation_stats["operation_times"]:
                avg_time = sum(self.operation_stats["operation_times"]) / len(
                    self.operation_stats["operation_times"]
                )
    ...
    def validate_assembly_components(components: list) -> bool:
        ...
        for component in components:
            ...
            if component["component_type"] not in [
                ComponentType.FINISHED_UNIT.value,
                ComponentType.FINISHED_GOOD.value,
            ]:
                return False
