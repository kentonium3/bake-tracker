...
def test_fifo_multiple_lots_partial_consumption(test_db):
  ...
  assert abs(result["consumed"] - Decimal("48.0")) < Decimal("0.001"), f"Expected 48.0 consumed, got {result["consumed"]}"
  assert result["satisfied"] is True
  assert abs(result["shortfall"] - Decimal("0.0")) < Decimal("0.001"), f"Expected 0.0 shortfall, got {result["shortfall"]}"
  ...
  assert abs(result["breakdown"][0]["remaining_in_lot"]) < Decimal("0.001"), f"Expected 0.0, got {result["breakdown"][0]["remaining_in_lot"]}"

  # Second lot partially consumed (15 lb - 2 lb = 13 lb remaining)
  assert result["breakdown"][1]["lot_date"] == date(2025, 1, 15)
  assert abs(result["breakdown"][1]["remaining_in_lot"] - Decimal("13.0")) < Decimal("0.001"), f"Expected 13.0, got {result["breakdown"][1]["remaining_in_lot"]}"
...
def test_fifo_insufficient_inventory(test_db):
  ...
  assert abs(result["consumed"] - Decimal("40.0")) < Decimal("0.001"), f"Expected 40.0 consumed, got {result["consumed"]}"  # Only 10 lb available
  assert result["satisfied"] is False
  assert abs(result["shortfall"] - Decimal("20.0")) < Decimal("0.001"), f"Expected 20.0 shortfall, got {result["shortfall"]}"  # 60 - 40 = 20 cups short
...
def test_fifo_exact_consumption(test_db):
  ...
  assert abs(result["consumed"] - Decimal("20.0")) < Decimal("0.001"), f"Expected 20.0 consumed, got {result["consumed"]}"
  assert result["satisfied"] is True
  assert abs(result["shortfall"] - Decimal("0.0")) < Decimal("0.001"), f"Expected 0.0 shortfall, got {result["shortfall"]}"
  assert abs(result["breakdown"][0]["remaining_in_lot"]) < Decimal("0.001"), f"Expected 0.0, got {result["breakdown"][0]["remaining_in_lot"]}"
...
def test_fifo_precision(test_db):
  ...
  variant = variant_service.create_variant(ingredient.slug, {
      "brand": "Bob's Red Mill",
      "purchase_unit": "oz",
      "purchase_quantity": Decimal("24.0")
  })
  ...
  assert abs(result["consumed"] - Decimal("5.75")) < Decimal("0.001"), f"Expected 5.75 consumed, got {result["consumed"]}"
  assert abs(result["breakdown"][0]["remaining_in_lot"] - Decimal("11.55")) < Decimal("0.001"), f"Expected 11.55, got {result["breakdown"][0]["remaining_in_lot"]}"  # 17.3 - 5.75