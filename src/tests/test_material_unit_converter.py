...
class TestUnitCompatibilityValidation:
    ...
    def test_validate_linear_units(self):
        """Test linear unit validation - should not raise."""
        validate_unit_compatibility("feet", "linear_cm")  # No exception = success
        validate_unit_compatibility("inches", "linear_cm")
        validate_unit_compatibility("cm", "linear_cm")

    def test_validate_area_units(self):
        """Test area unit validation - should not raise."""
        validate_unit_compatibility("square_feet", "square_cm")
        validate_unit_compatibility("square_inches", "square_cm")

    def test_validate_each_unit(self):
        """Test each unit validation - should not raise."""
        validate_unit_compatibility("each", "each")
...
class TestAreaToBaseConversions:
    ...
    def test_square_feet_to_square_cm(self):
        ...
        result = convert_to_base_units(Decimal("1"), "square_feet", "square_cm")
...
class TestLinearFromBaseConversions:
    ...
    def test_cm_to_feet(self):
        ...
        result = convert_from_base_units(Decimal("30.48"), "feet", "linear_cm")
        assert result == Decimal("1")

    def test_cm_to_inches(self):
        """Test cm to inches conversion: 2.54 cm = 1 inch."""
        result = convert_from_base_units(Decimal("2.54"), "inches", "linear_cm")
        assert result == Decimal("1")

    def test_cm_to_yards(self):
        """Test cm to yards conversion: 91.44 cm = 1 yard."""
        result = convert_from_base_units(Decimal("91.44"), "yards", "linear_cm")
        assert result == Decimal("1")

    def test_cm_to_meters(self):
        """Test cm to meters conversion: 100 cm = 1 meter."""
        result = convert_from_base_units(Decimal("100"), "meters", "linear_cm")
        assert result == Decimal("1")

    def test_cm_to_mm(self):
        """Test cm to mm conversion: 1 cm = 10 mm."""
        result = convert_from_base_units(Decimal("1"), "mm", "linear_cm")
        assert result == Decimal("10")
...
class TestAreaFromBaseConversions:
    ...
    def test_square_cm_to_square_feet(self):
        ...
        result = convert_from_base_units(
...
class TestConversionErrors:
    ...
    def test_negative_quantity_to_base(self):
        ...
        with pytest.raises(ConversionError) as exc:
            convert_to_base_units(Decimal("-5"), "feet", "linear_cm")
    ...
    def test_incompatible_area_to_linear(self):
        ...
        with pytest.raises(ConversionError) as exc:
            convert_units(Decimal("5"), "square_feet", "feet")
        assert "incompatible" in str(exc.value).lower()

    def test_unknown_from_unit(self):
        """Test unknown source unit error."""
        with pytest.raises(ConversionError) as exc:
            convert_units(Decimal("5"), "unknown", "feet")
        assert "unknown unit" in str(exc.value).lower()

    def test_unknown_to_unit(self):
        """Test unknown target unit error."""
        with pytest.raises(ConversionError) as exc:
            convert_units(Decimal("5"), "feet", "unknown")
        assert "unknown unit" in str(exc.value).lower()
...
class TestEdgeCases:
    ...
    def test_round_trip_conversion(self):
        ...
        # Convert to base
        base = convert_to_base_units(original, "feet", "linear_cm")

        # Convert back
        result = convert_from_base_units(base, "feet", "linear_cm")
