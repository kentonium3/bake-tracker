...
**Created**: 2026-01-16  
**Status**: Draft  
**Input**: User feedback: "The Edit Recipe yield area has redundant yield data in two places. The top-level yield_quantity/yield_unit duplicates what should be in yield type rows. Need unified 3-field yield rows (description, unit, quantity) with at least one required."
...
1. **Given** an export file with recipes containing yield_description, **When** the transformation script runs, **Then** each recipe creates one FinishedUnit with:
   - display_name = yield_description (e.g., "Large Sugar Cookie")
   - item_unit = yield_unit (e.g., "cookie")
   - items_per_batch = yield_quantity (e.g., 24)
   - slug = unique identifier (e.g., "sugar_cookie_large_sugar_cookie")
2. **Given** an export file with recipes WITHOUT yield_description, **When** the transformation script runs, **Then** each recipe creates one FinishedUnit with:
   - display_name = "Standard {recipe_name}" (e.g., "Standard Chocolate Chip Cookie")
   - item_unit = yield_unit (e.g., "cookie")
   - items_per_batch = yield_quantity (e.g., 36)
   - slug = unique identifier (e.g., "chocolate_chip_cookie_standard")
...
**Why this priority**: UI simplification improves usability, but the underlying data migration (P1) must happen first.
...
- **What happens when a recipe has yield_quantity/yield_unit but no FinishedUnits?** Transformation script creates a FinishedUnit from the legacy data using display_name generation logic (see User Story 3).
...
- **FR-001**: Recipe model MUST deprecate `yield_quantity`, `yield_unit`, and `yield_description` fields (mark nullable for migration phase)
- **FR-002**: FinishedUnit model MUST have `item_unit` field to store the unit type (e.g., "cookie", "cake", "slice")
- **FR-003**: FinishedUnit model MUST retain `display_name` field for yield description (e.g., "Large Cookie", "Standard Chocolate Chip Cookie")
...
- **FR-024**: Transformation script MUST generate unique `slug` for each FinishedUnit using pattern `{recipe_slug}_{yield_type_suffix}` where:
  - recipe_slug = slugify(recipe_name)
  - yield_type_suffix = slugify(yield_description) if present, else "standard"
  - Example: "chocolate_chip_cookie_standard", "sugar_cookie_large_sugar_cookie"
