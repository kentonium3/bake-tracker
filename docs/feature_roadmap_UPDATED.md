...
|---|------|----------|--------------|--------|
| 034 | Ingredient Hierarchy Phase 2 (Integration Fixes) | HIGH | F031, F032, F033 | Ready |
...
27. **Feature 034** - Ingredient Hierarchy Phase 2 (Integration Fixes)
...
### Feature 034: Ingredient Hierarchy Phase 2 (Integration Fixes)

**Status:** PLANNED (Ready to implement)

**Dependencies:** Features 031, 032, 033 (Phase 1)

**Problem:** Phase 1 fixed core ingredient management but integration gaps remain. Cascading filters broken in Product/Inventory tabs (L1 doesn't update when L0 changes). Product edit form hangs when opened. Recipe integration status completely unknown (likely missing cascading selectors).

**Solution:** Fix cascading filter logic across all tabs, debug product edit hang, verify and implement recipe integration.

**Planned Deliverables:**

1. **Fix Cascading Filters:**
   - Product tab filter: L1 dropdown updates when L0 changes
   - Inventory tab filter: Same fix
   - Add event handler guards to prevent infinite loops
   - Add "Clear All" button to reset filters
   - Estimate: 4-6 hours

2. **Debug Product Edit Form Hang:**
   - Investigate dialog rendering issue (exists in taskbar but content not visible)
   - Fix cascading dropdown logic causing hang/freeze
   - Add re-entry guards to event handlers
   - Thorough testing
   - Bug spec exists: `/docs/bugs/BUG_product_edit_hang.md`
   - Estimate: 4-8 hours

3. **Verify Recipe Integration:**
   - Test recipe creation ingredient selection workflow
   - Implement cascading selectors if missing
   - Add validation ensuring only L2 ingredients selectable
   - Verify recipes tab filtering by hierarchy works
   - Estimate: 4-6 hours

**Requirements Coverage:**
- REQ-ING-017: Product edit form cascading selector works
...
### Feature 033: Ingredient Hierarchy Gap Analysis - Phase 1
...
**Solution:** Three-tier hierarchical ingredient taxonomy with self-referential structure and leaf-only product assignment.
...
- Service layer: `ingredient_hierarchy_service.py` with tree traversal, ancestry queries, cycle prevention, level validation
- Import/export support: Handles hierarchical data correctly
...
**User Impact:**
- Enables hierarchical ingredient organization (foundation for UI work)
- Product-ingredient relationships clarified via hierarchy levels
- Import/export maintains hierarchy structure
...
### Feature 032: Ingredient Hierarchy UI Completion
...
**Dependencies:** F031 (Ingredient Hierarchy - schema and services)

**Problem:** F031 implemented backend support for ingredient hierarchy, but UI still used deprecated `category` field. Users could not effectively browse, filter, or manage the hierarchy. Multiple tabs showed category columns/filters instead of hierarchy.

**Solution:** Complete UI implementation across all affected tabs and forms, replacing category-based UI with hierarchy-based UI.

**Delivered:**
- **Ingredients Tab:** Three hierarchy columns (L0/L1/L2) replace category column, hierarchy level filter, search across all levels
- **Ingredient Edit Form:** Cascading dropdowns for L0/L1 selection, leaf ingredient radio button, proper validation
- **Products Tab:** Hierarchical ingredient filter (L0→L1→L2 cascading), full ingredient path display
- **Inventory Tab:** Hierarchy columns and filters replace category, ingredient path display
- **Inventory Edit Form:** Read-only hierarchy display (three levels) instead of greyed-out category field
- **Deprecated Field Removal:** All UI references to `category` field removed

**User Impact:**
- Can browse ingredients hierarchically (L0 → L1 → L2 navigation)
- Can create/edit ingredients at any hierarchy level
- Can filter products/inventory by ingredient hierarchy
- Full visibility into ingredient organization across all tabs
- Deprecated category field no longer confuses users

**Bug Fix Document:** `docs/bugs/BUG_F031_incomplete_hierarchy_ui.md`

---

### Feature 033: Recipe Redesign

**Status:** PLANNED (Phase 2 Architecture Complete)

**Dependencies:** F031, F032 (Ingredient Hierarchy - recipes use hierarchical selection)
...
### Feature 034: UI Mode Restructure
...
### Feature 035: Purchase Workflow & AI Assist

**Status:** PLANNED (Phase 2 Architecture Complete)

**Dependencies:** F034 (UI Mode Restructure - Purchases tab in SHOP mode)
...
### Feature 036: Finished Goods Inventory

**Status:** PLANNED (Phase 2 Architecture Complete - Service Layer Only, No UI)

**Dependencies:** F033 (Recipe Redesign - production runs create snapshots)
...
### Feature 037: Packaging & Distribution
...
- **Phases 2-4 Planned:** Remaining gap analysis work divided into three features:
  - **F034 (Phase 2 - Integration Fixes):** Fix cascading filters, debug product edit hang, verify recipe integration. 12-20 hours estimated.
...
- **Feature Renumbering:** Inserted F034-F036 for gap analysis phases. Previous planned features renumbered:
  ...
  - Purchase Workflow & AI Assist: F036 → F039
  - Finished Goods Inventory: F037 → F040
...
- **Feature 033 Scoped:** Gap analysis created identifying F031+F032 55% incomplete after Phase 1. Documented 5 critical blockers, missing service methods, mapped all requirements. Divided remaining work into 4 phases (50-72 hours total). Phase 1 to be implemented immediately, Phases 2-4 deferred to F034-F036.
- **Phase 2 Architecture Complete:** Five comprehensive design specifications created for Phase 2 requirements:
  - **F031 (Ingredient Hierarchy):** Three-tier hierarchical taxonomy (L0/L1/L2), leaf-only product assignment, AI-assisted migration from flat categories.
  - **F037 (Recipe Redesign):** Template/snapshot architecture for immutable production history, base/variant relationships via self-referential FK, simple scaling (1x/2x/3x multiplier). Depends on F031, F036.
  - **F038 (UI Mode Restructure):** 5-mode workflow organization (CATALOG/PLAN/SHOP/PRODUCE/OBSERVE), consistent tab layouts, mode-specific dashboards. UI-only, no schema changes.
  - **F039 (Purchase Workflow & AI Assist):** Multi-channel purchase capture (manual/CSV/AI Studio), UPC matching with learned mappings, batch file import for Phase 2, real-time API for Phase 3. Depends on F038.
  - **F040 (Finished Goods Inventory):** Service layer architecture for inventory management and validation. Data model exists, UI deferred to Phase 3. Depends on F037.
- **Feature Renumbering:** Original F031 (Packaging & Distribution) moved to F041. Gap analysis Phases 2-4 will be F034-F036.
